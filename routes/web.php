...
use App\Http\Controllers\Web\Account\JobPreferencesController;
use App\Http\Controllers\Web\Account\JobMatchesController;
...
use App\Http\Controllers\Web\InterviewTipsController;
use App\Http\Controllers\Web\CourseController;
use App\Http\Controllers\Web\EnrollmentController;
use App\Http\Controllers\Web\LessonController;
use App\Http\Controllers\Web\ProductAdController;
use App\Http\Controllers\Web\AdvertiseController;
use App\Http\Controllers\Web\NotificationController;
use App\Http\Controllers\Web\PostReactionController;
...
Route::namespace('App\Http\Controllers\Web')
	...
	->group(function ($router) {
		...
		if (!doesCountriesPageCanBeHomepage()) {
			Route::get('/', [HomeController::class, 'index'])->middleware('cache.response:600');
			Route::get(dynamicRoute('routes.countries'), [CountriesController::class, 'index'])->middleware('cache.response:600');
		} else {
			Route::get('/', [CountriesController::class, 'index'])->middleware('cache.response:600');
		}
			...
			->group(function ($router) {
				...
				Route::middleware(['auth', 'banned.user', 'no.http.cache'])
					->group(function ($router) {
						...
						// Job Preferences
						Route::prefix('job-preferences')
							->controller(\App\Http\Controllers\Web\Account\JobPreferencesController::class)
		...
		Route::namespace('Ajax')
			...
			->group(function ($router) {
				...
				Route::controller(PostReactionController::class)
					->prefix('reactions')
					->group(function ($router) {
						Route::post('post/{postId}/toggle', 'toggle')->where('postId', '[0-9]+');
						Route::get('post/{postId}', 'show')->where('postId', '[0-9]+');
		...
		Route::prefix('courses')->group(function () {
			...
			Route::middleware('auth')->group(function () {
				...
				Route::get('/my/enrolled', [CourseController::class, 'myCourses'])->name('courses.my');

				// Enrollment
				Route::post('/{courseId}/enroll', [EnrollmentController::class, 'enroll'])->name('courses.enroll');
				Route::delete('/{courseId}/unenroll', [EnrollmentController::class, 'unenroll'])->name('courses.unenroll');

				// Lessons
				Route::get('/{courseSlug}/lessons/{lessonId}', [LessonController::class, 'show'])->name('courses.lessons.show');
				Route::post('/lessons/{lessonId}/complete', [LessonController::class, 'complete'])->name('courses.lessons.complete');

				// Interactive Lessons
				Route::get('/{courseSlug}/lessons/{lessonId}/interactive', [LessonController::class, 'interactive'])->name('courses.lessons.interactive');
			});
		});
		...
		Route::prefix('ad')
			->controller(ProductAdController::class)
			->group(function () {
				Route::get('click/{id}', 'click')->where('id', '[0-9]+');
				Route::post('impression/{id}', 'impression')->where('id', '[0-9]+');
			});

		// ADVERTISE WITH US - Product Advertising
		Route::prefix('advertise')
			->controller(AdvertiseController::class)
			->group(function () {
				Route::get('/', 'index')->name('advertise.index')->middleware('cache.response:600');
				Route::get('/create/{packageId}', 'create')->name('advertise.create')->where('packageId', '[0-9]+');
				Route::post('/store', 'store')->name('advertise.store');
				Route::get('/payment/{adId}/{packageId}', 'payment')->name('advertise.payment')->where(['adId' => '[0-9]+', 'packageId' => '[0-9]+']);
				Route::post('/payment/callback', 'paymentCallback')->name('advertise.payment.callback');
				Route::get('/my-ads', 'myAds')->name('advertise.my-ads');
				Route::post('/{id}/pause', 'pause')->name('advertise.pause')->where('id', '[0-9]+');
				Route::post('/{id}/resume', 'resume')->name('advertise.resume')->where('id', '[0-9]+');
			});
		...
		Route::prefix('account/notifications')
			...
			->group(function () {
				...
				Route::get('/unsubscribe/{encodedEmail}', 'unsubscribe')->name('notifications.unsubscribe');
				Route::post('/unsubscribe/{encodedEmail}', 'processUnsubscribe')->name('notifications.process-unsubscribe');

				// Authenticated routes
				Route::middleware('auth')->group(function () {
					Route::get('/settings', 'settings')->name('notifications.settings');
					Route::post('/toggle', 'toggleNotifications')->name('notifications.toggle');
				});
			});

		// JOB PREFERENCES - Job Matching & Auto-Apply Settings
		Route::prefix('account/job-preferences')
			->controller(\App\Http\Controllers\Web\Account\JobPreferencesController::class)
			->middleware('auth')
			->group(function () {
				Route::get('/', 'index')->name('job-preferences.index');
				Route::post('/', 'store')->name('job-preferences.store');
				Route::put('/', 'update')->name('job-preferences.update');
			});

		// JOB MATCHES - View and Manage Job Matches
		Route::prefix('account/job-matches')
			->controller(\App\Http\Controllers\Web\Account\JobMatchesController::class)
			->middleware('auth')
			->group(function () {
				Route::get('/', 'index')->name('job-matches.index');
				Route::get('/{match}', 'show')->name('job-matches.show');
				Route::post('/{match}/apply', 'apply')->name('job-matches.apply');
				Route::post('/{match}/reject', 'reject')->name('job-matches.reject');
				Route::post('/{match}/approve', 'approve')->name('job-matches.approve');
			});

		// SUBSCRIPTIONS - Manage Subscription Plans
		Route::prefix('account/subscriptions')
			->controller(\App\Http\Controllers\Web\Account\SubscriptionController::class)
			->middleware('auth')
			->group(function () {
				Route::get('/', 'index')->name('subscriptions.index');
				Route::get('/checkout/{planSlug}', 'checkout')->name('subscriptions.checkout');
				Route::post('/subscribe', 'subscribe')->name('subscriptions.subscribe');
				Route::get('/payment/{planId}/confirmation', 'paymentConfirmation')->name('subscriptions.payment.confirmation');
				Route::get('/payment/{planId}/success', 'paymentSuccess')->name('subscriptions.payment.success');
				Route::get('/payment/{planId}/cancel', 'paymentCancel')->name('subscriptions.payment.cancel');
				Route::post('/cancel', 'cancel')->name('subscriptions.cancel');
			});

		// SEARCH
		Route::group(['namespace' => 'Search', 'middleware' => 'throttle:search'], function ($router) {
			$router->pattern('id', '[0-9]+');
