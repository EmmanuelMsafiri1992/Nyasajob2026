...
class StyleSetting
{
	...
	public static function getValues($value, $disk)
	{
		...
		$value['body_background_image_url'] = !empty($bodyBackgroundImage) ? imgUrl($bodyBackgroundImage, 'bg-body') : null;
		
		// login_bg_image_url
		$loginBgImage = $value['login_bg_image'] ?? config('larapen.admin.login_bg_image', '');
		$value['login_bg_image_url'] = imgUrl($loginBgImage, 'bg-body');
