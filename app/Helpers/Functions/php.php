 ...
 * @return string
 */
function strStrip(?string $string): string
{
	return trim(preg_replace('/\s\s+/u', ' ', strval($string)));
}
 ...
 * @return string
 */
function mbStrCleaner(?string $string): string
{
	$string = strip_tags(strval($string), '<br><br/>');
...
{
	$string = strip_tags(strval($string));
 ...
 * @return string|null
 */
function strRemoveNumeric(?string $string): ?string
{
	$string = preg_replace('/\d/u', '', strip_tags(strval($string)));
...
{
	$string = strRemoveSpecialChars($string);
	$string = strRemoveWhitespace($string);
 ...
 * @return string
 */
function strRemoveWhitespace(?string $string, string $replacement = ''): string
{
	// White-space = [ \t\r\n\f];
	$pattern = '/\s+/u';
	$string = preg_replace($pattern, $replacement, strval($string));
 ...
 * $cleanedString = strRemoveWhitespace($inputString);
 ...
 * @return string
 */
function strRemoveSpecialChars(?string $string, string $replacement = ''): string
{
 ...
 * @return string
 */
function stripNonUtf8Chars(?string $string): string
{
	/*
 ...
 * @return string
 */
function changeWhiteSpace(?string $string): string
{
	return getAsString(str_replace(PHP_EOL, ' ', strval($string)));
}
 ...
 * @return bool
 */
function isRealDirectory(string $dirPath): bool
{
 ...
 *
 * Converts characters like "café" to "cafe", "über" to "uber", etc.
 * This is useful for diacritic-insensitive search functionality.
 *
 * @param string|null $string
 * @return string
 */
function removeDiacritics(?string $string): string
{
	...
	// Use Transliterator if available (requires intl extension)
	...
	// Fallback: Manual character mapping for common diacritics
	$diacritics = [
		...
		'Ŏ' => 'O', 'ŏ' => 'o', 'Ő' => 'O', 'ő' => 'o', 'Ŕ' => 'R', 'ŕ' => 'r', 'Ŗ' => 'R',
		'ŗ' => 'r', 'Ř' => 'R', 'ř' => 'r', 'Ś' => 'S', 'ś' => 's', 'Ŝ' => 'S', 'ŝ' => 's',
		'Ş' => 'S', 'ş' => 's', 'Ţ' => 'T', 'ţ' => 't', 'Ť' => 'T', 'ť' => 't', 'Ŧ' => 'T',
...
/**
 * Check if diacritic-sensitive search is enabled
 *
 * @return bool
 */
function isDiacriticSensitiveSearchEnabled(): bool
{
	return config('settings.listings_list.diacritic_sensitive_search') == '1';
}
