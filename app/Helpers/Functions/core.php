...
{
	return (
		str_contains(currentRouteAction(), 'Web\Install\\')
...
function isAdminPanel(?string $url = null): bool
{
	if (empty($url)) {
		...
		try {
			...
			$adminUri = str(admin_uri())->start('/')->toString();
...
function isDemoDomain(?string $url = null): bool
{
	...
	if (auth()->check()) {
		...
		if (
			...
			&& md5($authUser->getAuthIdentifier()) == 'c4ca4238a0b923820dcc509a6f75849b'
		) {
			$isDemoDomain = false;
		}
	}
...
{
	$skinsArray = getReferrerList('skins');
	...
	if (!file_exists($filePath)) {
		try {
			...
			$img = \Intervention\Image\Facades\Image::canvas(70, 35, $skinInfo['color']);
 ...
 * @return string
 */
function transformDescription($string): string
{
	if (config('settings.listing_form.wysiwyg_editor') != 'none') {
		...
		$string = nl2br(urlsToLinks(mbStrCleaner($string)));
	}
 ...
 */
function tagCleaner($tagString, bool $asArray = false): array|string|null
{
	$limit = (int)config('settings.listing_form.tags_limit', 15);
	
	return taggable($tagString, $limit, $asArray);
}
 ...
 * @return string
 */
function getUploadFileTypes(?string $uploadType = 'file', bool $jsFormat = false): string
{
	if ($uploadType == 'image') {
	...
	if ($jsFormat) {
		$types = explode(',', $types);
		$types = array_filter($types, fn ($value) => $value !== '');
		$types = json_encode($types);
	}
	
	return getAsString($types);
}
 ...
 * @return string
 */
function showValidFileTypes(?string $uploadType = 'file'): string
{
	$formats = getUploadFileTypes($uploadType);
	$formats = str_replace(',', ', ', $formats);
	
	return getAsString($formats);
}
 ...
 * @return string
 */
function imgTag(?string $filePath, ?string $resizeOptionsName = 'picture-lg', ?array $attr = []): string
{
	$src = imgUrl($filePath, $resizeOptionsName);
	$attr = buildAttributes($attr);
	
	$out = '';
	if (config('settings.optimization.webp_format')) {
		$srcWebp = imgUrl($filePath, $resizeOptionsName, true);
 ...
 * @return string
 */
function imgUrl(?string $filePath, ?string $resizeOptionsName = 'picture-lg', bool $webpFormat = false): string
{
	// Storage Disk Init.
	$disk = \App\Helpers\Files\Storage\StorageDisk::getDisk();
	...
	if (!in_array($resizeOptionsName, $resizeOptionsNamesArray)) {
		try {
			return $disk->url($filePath) . getPictureVersion();
		} catch (\Exception $e) {
	...
	if ($webpFormat) {
		return resizeWebp($disk, $filePath, $resizeOptionsName);
	} else {
		return resize($disk, $filePath, $resizeOptionsName);
	}
}
 ...
 * @return string
 */
function resize($disk, ?string $filePath, ?string $resizeOptionsName = 'picture-lg', bool $webpFormat = false): string
{
	// Image Quality (for JPEG)
	$imageQuality = config('settings.upload.image_quality', 90);
	
	// Resize Parameters (Default)
	...
	if (preg_match('/\dx\d/i', $resizeOptionsName)) {
		...
		$width = (int)($tmp[0] ?? $width);
		$height = (int)($tmp[1] ?? $height);
	} else {
		// Resize Parameters (from Admin Settings)
		$absSettingKeyPrefix = 'settings.upload.img_resize_' . str_replace('-', '_', $resizeOptionsName);
		$width = (int)config($absSettingKeyPrefix . '_width', $width);
		$height = (int)config($absSettingKeyPrefix . '_height', $height);
		$method = config($absSettingKeyPrefix . '_method', $method);
		$ratio = config($absSettingKeyPrefix . '_ratio', $ratio);
		$upsize = config($absSettingKeyPrefix . '_upsize', $upsize);
		$position = config($absSettingKeyPrefix . '_position', $position);
		$relative = config($absSettingKeyPrefix . '_relative', $relative);
		$bgColor = config($absSettingKeyPrefix . '_bgColor', $bgColor);
	}
	...
	$fileDir = str($fileDir)->finish(DIRECTORY_SEPARATOR)->toString();
	...
	if (!$disk->exists($thumbFilePath)) {
		...
		try {
			...
			if ($webpFormat) {
				...
				$extension = isPng($disk->get($filePath)) ? 'png' : 'jpg';
			}
 ...
 * @return string
 */
function resizeWebp($disk, ?string $filePath, ?string $resizeOptionsName = 'picture-lg'): string
{
	return resize($disk, $filePath, $resizeOptionsName, true);
}
...
function getTokenLabel(): string
{
	...
	$label = t('Code received by SMS or Email');
	if ($authField == 'email') {
		$label = t('Code received by Email');
	}
	if ($authField == 'phone') {
		$label = t('Code received by SMS');
	}
...
function getTokenMessage(): string
{
	...
	$message = t('Enter the code you received by SMS or Email in the field below');
	if ($authField == 'email') {
		$message = t('Enter the code you received by Email in the field below');
	}
	if ($authField == 'phone') {
		$message = t('Enter the code you received by SMS in the field below');
	}
...
/**
 * Does User can create listing?
 *
 * @param $user
 * @return bool
 */
function doesUserCanCreateListing($user): bool
{
	return (
		empty($user)
		|| (
...
function getCreateListingLinkInfo(): array
{
	...
	$userCanCreateListing = doesUserCanCreateListing($authUser);
	
	$linkUrl = '';
	$linkAttr = '';
	
	if ($userCanCreateListing) {
		$linkUrl = \App\Helpers\UrlGen::addPost();
		...
		if (config('settings.listing_form.pricing_page_enabled') == '1') {
			 ...
			 */
				...
				if (empty($authUser->payment)) {
					$linkUrl = \App\Helpers\UrlGen::pricing();
				}
			} else {
	...
	return [$userCanCreateListing, $linkUrl, $linkAttr];
}
...
function genPhoneNumberBtn($post, bool $btnBlock = false): string
{
	...
	if (config('settings.listing_page.hide_phone_number')) {
		...
		if (config('settings.listing_page.hide_phone_number') == '1') {
			$phone = str($phoneToHide)->mask('X', -str($phoneToHide)->length(), str($phoneToHide)->length() - 3);
		}
		if (config('settings.listing_page.hide_phone_number') == '2') {
			$phone = str($phoneToHide)->mask('X', 3);
		}
		if (config('settings.listing_page.hide_phone_number') == '3') {
...
function getFilterClearBtn(?string $url): ?string
{
	...
	if (!empty($url)) {
		...
		$out .= '<i class="fa-regular fa-rectangle-xmark" style="float: ' . $float . '; margin-top: 2px; color: #999;"></i>';
		$out .= '</a>';
	}
 ...
 * @return float|int
 */
function getNumberOfItemsPerPage(?string $entity = null, $itemsPerPage = null): float|int
{
	$entity = getAsString($entity);
	
	$isPerPageValid = (is_numeric($itemsPerPage) && $itemsPerPage > 0);
	if (!$isPerPageValid) {
		$defaultPerPage = 10;
		$perPage = config('settings.pagination.per_page', $defaultPerPage);
		$itemsPerPage = config('settings.pagination.' . $entity . '_per_page', $perPage);
		$itemsPerPage = (is_numeric($itemsPerPage) && $itemsPerPage > 0)
			? $itemsPerPage
			: $defaultPerPage;
	}
	
	return Number::clamp($itemsPerPage, min: 1, max: getMaxItemsPerPage($entity));
}
...
function getMaxItemsPerPage(?string $entity = null): float|int
{
	...
	$maxItemsPerPage = ($entity == 'posts')
		? env('MAX_POSTS_PER_PAGE', $defaultMaxItemsPerPage)
		: env('MAX_ITEMS_PER_PAGE', $defaultMaxItemsPerPage);
