...
class SitemapsController extends FrontController
{
	...
	public function getCitiesSitemapByCountry(string $countryCode = null)
	{
		...
		if ($cities->count() > 0) {
			foreach ($cities as $city) {
				...
				$locale = is_object($country) ? $country->locale : ($country['locale'] ?? null);
				$icode = is_object($country) ? $country->icode : ($country['icode'] ?? null);
				$url = UrlGen::city($city, $locale, $icode);
  ...
  public function getCountrySettings(?string $countryCode, bool $canApplySettings = true): ?array
  {
      ...
      // Get Country Info
      $country = collect(CountryLocalization::getCountryInfo($countryCode));
      if ($country->isEmpty()) {
          return null;
      }
      ...
      $tab['time_zone'] = $country->get('time_zone', config('app.timezone'));
