...
class LocaleController extends FrontController
{
	...
	public function setLocale(string $langCode)
	{
		...
		// Create the cookie to attach to redirect response
		$langCookie = $this->createLangCookie($langCode, $langCanBeSaved);

		// Save the Language Code (for session-based storage)
		...
		if (request()->filled('from')) {
			...
			if (!empty($path) && !str_contains($path, '#')) {
				...
				return $langCookie ? redirect($url)->withCookie($langCookie) : redirect($url);
			}
		}
		...
		if (request()->filled('country')) {
			...
			return $langCookie ? redirect($nextUrl)->withCookie($langCookie) : redirect($nextUrl);
		} else {
			...
			return $langCookie ? redirect($previousUrl)->withCookie($langCookie) : redirect($previousUrl);
		}
	}
	...
	/**
	 * Create a language cookie for attaching to redirect response
	 *
	 * @param string $langCode
	 * @param bool $langCanBeSaved
	 * @return \Symfony\Component\HttpFoundation\Cookie|null
	 */
	private function createLangCookie(string $langCode, bool $langCanBeSaved)
	{
		if (config('larapen.core.storingUserSelectedLang') != 'cookie') {
			return null;
		}
		...
		if ($langCanBeSaved) {
			...
			$path = !empty(config('session.path')) ? config('session.path') : '/';
			$domain = !empty(config('session.domain')) ? config('session.domain') : getCookieDomain();
			$secure = config('session.secure');
			$httpOnly = config('session.http_only');
			$sameSite = config('session.same_site');

			return cookie()->make('langCode', $langCode, $expires, $path, $domain, $secure, $httpOnly, false, $sameSite);
		} else {
			// Return a cookie that expires immediately to delete it
			return cookie()->forget('langCode');
		}
	}
